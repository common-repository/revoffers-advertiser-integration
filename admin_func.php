...
function init_admin()
{
    add_action('admin_init', function () {
        ...
        register_setting('revoffers', 'revoffers_use_cron', [
            'type' => 'boolean',
            'description' => 'Whether to force RevOffers plugin to use the cron subsystem even if WP_DISABLE_CRON is activated',
        ]);
    });

    add_action('admin_menu', function () {
        // Creates admin menu item, if user has correct permissions
        $pageTitle = 'RevOffers Network Settings';
        $menuTitle = 'RevOffers Network';
        $urlSlug = pluginName();
        add_options_page($pageTitle, $menuTitle, 'manage_options',
            $urlSlug, __NAMESPACE__ . '\\admin_options_page');
